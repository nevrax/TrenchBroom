...
namespace TrenchBroom {
    namespace View {
        ...
        SwapNodeContentsCommand::SwapNodeContentsCommand(const std::string& name, std::vector<std::tuple<Model::Node*, std::unique_ptr<Model::Node>>> nodes) :
        ...
        bool SwapNodeContentsCommand::doCollateWith(UndoableCommand* command) {
            ...
            auto myNodes = kdl::vec_transform(m_nodes, [](const auto& t) { return std::get<0>(t); });
            auto theirNodes = kdl::vec_transform(other->m_nodes, [](const auto& t) { return std::get<0>(t); });
